...
") %>%
  mutate(
    ...
    prediction = ifelse(EPTEST_ANY > 0, "50", "98")
  )
```
...
") %>%
  ...
  mutate(
    correct = prediction == LOC,
    type1 = prediction == "50" & LOC == "98",
    type2 = prediction == "98" & LOC == "50"
  )
```
...
### Predictions
```{r}
results <- table(testCases$LOC, testCases$prediction) %>%
  apply(c(1, 2), function (x) {
    paste0(comma(x), " (", percent(x / nrow(testCases)), ")")
  })
rownames(results) <- c("Actual: 50", "Actual: 98")
colnames(results) <- c("Predicted: 50", "Predicted: 98")
kable(results)
```

### Type I errors by test
```{r}
positive <- colSums(testCases[,3:(length(testCases) - 4)])
type1_error <- colSums(testCases[testCases$type1,3:(length(testCases) - 4)])
...
* Private attorney: do they handle "Agent" cases and cases with representation by the National Veterans Legal Services Program (a private organization).
* We need to look further into Manila Remand/Filipino Veterans Equity Compensation Fund. How do they determine what qualifies?
* We need to check whether AMC handles compensation overpayments.
