...
Show the share of appeals that are certified using Caseflow. Report run 02/22/17.
...
certifications <- dbGetQuery(con, "
  ...
  where BF41STAT >= date '2016-01-01' and BF41STAT < date '2017-02-01'
  and BFAC = '1'
") %>%
  mutate(
...
certifications.eligible_by_ro <- certifications %>%
  ...
  filter(!caseflow) %>%
  group_by(RO) %>%
  filter(n() == length(unique(certifications$month))) %>%
  ungroup() %>%
  mutate(rate = (total - n) / total)
